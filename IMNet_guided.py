...
def train(net, n_dataset, tolerance, max_cycle=1e6):
    ...
    while errorsum > tolerance and cycle < max_cycle:
        ...
        if errorsum >= old_errorsum:
            net.layers = old_layers
            # print("Cycle:", cycle, "Errorsum:", old_errorsum)
        else:
            if utype == "new":
                print("Cycle:", cycle, "AMP:", round(amp, 4), "New Delta - Rate", round(urate, 5))
            else:
                print("Cycle:", cycle, "AMP:", round(amp, 4), "Delta Reapply - Rate", round(urate, 5))
